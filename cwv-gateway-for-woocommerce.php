 ...
 *
 * Plugin Name: CWV Gateway for Woocommerce
 * Plugin URI: https://github.com/kassemitani/cwv-gateway-for-woocommerce
 * Description: Show prices in CWV Waves (or asset) and accept Waves payments in your woocommerce webshop
 * Version: 0.6.0
 * Author: KassemItani
 * License: GPLv2 or later
 * License URI: http://www.opensource.org/licenses/gpl-license.php
 * Text Domain: cwv-gateway-for-woocommerce
...
if (!class_exists('WcWaves')) {
    ...
    class WcWaves
    {
        ...
        public function WavesCurrencies( $currencies )
        {
            ...
            $currencies['CWV'] = __( 'Cryptowave', 'CWV' );
        ...
        public function WavesCurrencySymbols( $currency_symbol, $currency ) {
            switch( $currency ) {
                ...
                case 'CWV': $currency_symbol = 'CWV'; break;
	    ...
	    private function convertToWavesPrice($price_string, $price)
	    {
            ...
            if(!in_array(get_woocommerce_currency(), array("WAVES","CWV","WNET","ARTcoin")) && $options['show_prices'] == 'yes') {
...
function cwvGateway_textdomain() {
    load_plugin_textdomain( 'cwv-gateway-for-woocommerce', FALSE, basename( dirname( __FILE__ ) ) . '/languages/' );
}

add_action( 'plugins_loaded', 'cwvGateway_textdomain' );
